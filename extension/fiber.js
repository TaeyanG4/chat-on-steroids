...
(() => {
  ...
  function pluginSnapshot() {
    ...
    let result = null, control = null, observedActions = null;
    for (const button of buttons) {
      ...
      for (let up = 0; fiber && up < 24; up++, fiber = fiber.return) {
        ...
        if (props.reportEntity?.entityType === 'connector' && props.reportEntity.id === route[1] &&
            ...
            props.headerTrailingContent && button.getClientRects().length > 0) {
    ...
    if (control.getAttribute('data-clf-plugin-refresh') !== route[1]) control.setAttribute('data-clf-plugin-refresh', route[1]);
    return result;
  }
