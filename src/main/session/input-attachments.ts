...
/** Only explicit file selection/drop writes here; bytes never inflate the durable outbox. */
let staging: Promise<unknown> = Promise.resolve();
export function stageInputAttachment(source: string | { text: string }, retained: Set<string>): Promise<InputAttachment> {
  const next = staging.then(() => stage(source, retained));
  staging = next.catch(() => undefined);
  return next;
}
async function stage(source: string | { text: string }, retained: Set<string>): Promise<InputAttachment> {
  const dir = directory();
  ...
  const size = stat?.size ?? Buffer.byteLength((source as { text: string }).text);
  ...
  const name = typeof source === 'string' ? path.basename(source) : 'Attached text.txt';
  ...
  try {
    ...
    } else await fs.writeFile(destination, source.text, { flag: 'wx' });
